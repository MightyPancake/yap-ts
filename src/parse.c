...
yap_block yap_parse_block(yap_source* src, TSNode node){
    ...
    yap_log("Parsing block: %u statements, type: %s, errors: %s", child_count, ts_node_type(node), ts_node_error_or_null(node) ? "yes" : "no");
    darr(yap_statement) statements = yap_ctx_darr_new(ctx, yap_statement, .cap=child_count, .len=0);
    for_ts_named_children(node, n){
